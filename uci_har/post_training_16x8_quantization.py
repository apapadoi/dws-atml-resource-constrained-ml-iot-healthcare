...
NUM_EPOCHS = 1000
BATCH_SIZE = 64
MODEL_OUTPUT_FOLDER = "../models/uci_har/post_training_16x8_quantization"
DEVICE = "/device:GPU:1"
...
with tf.device(DEVICE):
    ...
    tflite_16x8_model_quant = converter.convert()

    tflite_model_16x8_quant_file = tflite_models_dir/"model_quant.tflite"
    tflite_model_16x8_quant_file.write_bytes(tflite_16x8_model_quant)

    y_pred_initial_model = evaluate_model(tflite_model_file, X_test)
    y_pred_model_quant = evaluate_model(tflite_model_16x8_quant_file, X_test)

print("Initial model accuracy: ", accuracy_score(y_test, y_pred_initial_model))
print("16x8 quantized model accuracy: ", accuracy_score(y_test, y_pred_model_quant))

print("Initial model macro F1-score: ", f1_score(y_test, y_pred_initial_model, average='macro'))
print("16x8 quantized model macro F1-score: ", f1_score(y_test, y_pred_model_quant, average='macro'))
